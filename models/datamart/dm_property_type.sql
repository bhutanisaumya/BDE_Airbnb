...
WITH CTE_PROPERTY_TYPE AS (
    ...
    GROUP BY property_type, room_type, accommodates, TO_DATE(SUBSTRING(date, 1, 7), 'YYYY-MM')
)
...
SELECT 
    ...
    (total_active_listings - LAG(total_active_listings) OVER (PARTITION BY property_type, room_type, accommodates ORDER BY "month/year")) * 100 / NULLIF(LAG(total_active_listings) OVER (PARTITION BY property_type, room_type, accommodates ORDER BY "month/year"), 0) AS percentage_change_active,
    (total_listings - total_active_listings - (LAG(total_listings) OVER (PARTITION BY property_type, room_type, accommodates ORDER BY "month/year") - LAG(total_active_listings) OVER (PARTITION BY property_type, room_type, accommodates ORDER BY "month/year"))) * 100 / NULLIF((LAG(total_listings) OVER (PARTITION BY property_type, room_type, accommodates ORDER BY "month/year") - LAG(total_active_listings) OVER (PARTITION BY property_type, room_type, accommodates ORDER BY "month/year")), 0) AS percentage_change_inactive,
    total_number_of_stays,
    (total_revenue / NULLIF(total_listings, 0)) AS avg_rev
FROM CTE_PROPERTY_TYPE
